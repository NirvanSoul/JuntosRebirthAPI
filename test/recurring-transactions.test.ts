...
describe("recurring transactions",()=>{
 ...
 it("returns 400 instead of a generic 500 for an integrity failure",async()=>{const{app}=setup({createSeries:vi.fn().mockRejectedValue({cause:{code:"23514"}})});const r=await app.request("/v1/spaces/space/recurring-transactions",{method:"POST",body:JSON.stringify(monthly)},bindings);expect(r.status).toBe(400);await expect(r.json()).resolves.toMatchObject({error:{code:"INVALID_REQUEST"}})});
