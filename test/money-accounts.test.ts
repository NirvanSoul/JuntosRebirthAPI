...
describe("money account balances", () => {
  it("uses opening balance when there are no transactions", async () => {
    const accounts = await listMoneyAccounts(databaseWithRows([{
      id: "a", name: "Revolut", kind: "bank", icon: null, colorToken: null, primaryCurrency: "EUR", createdBy: "user-creator", createdAt: new Date(), balanceId: "b", balanceCurrency: "EUR", opening: 100000n, displayOrder: 0, transactionType: null, transactionAmount: null,
    }]), "space-1");
    expect(accounts[0]?.balances[0]?.currentBalanceMinor).toBe("100000");
    expect(accounts[0]?.createdBy).toBe("user-creator");
  });

  it("adds income and subtracts expense in the same balance currency", async () => {
    const base = { id:"a",name:"Revolut",kind:"bank" as const,icon:null,colorToken:null,primaryCurrency:"EUR",createdBy:"user-creator",createdAt:new Date(),balanceId:"b",balanceCurrency:"EUR",opening:100000n,displayOrder:0 };
    const accounts = await listMoneyAccounts(databaseWithRows([
