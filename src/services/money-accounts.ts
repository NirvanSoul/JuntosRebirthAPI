...
import { moneyAccountBalances, moneyAccounts, transactions } from "../db/schema";
...
export async function deleteBalance(db:Database,accountId:string,currency:string) { const [row] = await db.delete(moneyAccountBalances).where(and(eq(moneyAccountBalances.moneyAccountId,accountId),eq(moneyAccountBalances.currency,currency),notExists(db.select({one:sql`1`}).from(transactions).where(and(eq(transactions.moneyAccountId,accountId),eq(transactions.currency,currency),eq(transactions.isArchived,false),isNull(transactions.archivedAt)))))).returning({id:moneyAccountBalances.id}); return Boolean(row); }
