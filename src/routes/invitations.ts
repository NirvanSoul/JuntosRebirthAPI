...
export function createInvitationAcceptanceRoute(deps: Deps = defaults) { const route = new Hono<{ Bindings: Bindings; Variables: AuthVariables }>(); route.get("/", async c => { try { const db=deps.createDb(c.env.DATABASE_URL); const [current]=await db.select({email:user.email}).from(user).where(eq(user.id,c.get("currentUserId"))); return c.json({data:{invitations:current?await deps.listIncomingInvitations(db,c.get("currentUserId"),current.email):[]}}); } catch { return errorResponse(c, "INTERNAL_SERVER_ERROR"); } }); route.post("/accept", async c => { const body = await parseBody(c.req.raw, ["token"]); if (!body || typeof body.token !== "string" || !/^[a-f0-9]{64}$/.test(body.token)) return errorResponse(c, "INVALID_REQUEST"); try { const db=deps.createDb(c.env.DATABASE_URL); const spaceId = await deps.acceptInvitation(db, c.get("currentUserId"), body.token); if (spaceId) return c.json({ data: { spaceId } }); return await deps.invitationCountryMatches(db,c.get("currentUserId"),undefined,body.token) ? errorResponse(c,"INVITATION_NOT_FOUND") : errorResponse(c,"SPACE_COUNTRY_MISMATCH"); } catch { return errorResponse(c, "INTERNAL_SERVER_ERROR"); } }); route.post("/:invitationId/decline", async c => { try { const db=deps.createDb(c.env.DATABASE_URL); const [current]=await db.select({email:user.email}).from(user).where(eq(user.id,c.get("currentUserId"))); if(!current) return errorResponse(c, "INVITATION_NOT_FOUND"); const declined=await deps.declineInvitation(db,c.get("currentUserId"),current.email,c.req.param("invitationId")!); return declined?c.json({data:{declined:true}}):errorResponse(c, "INVITATION_NOT_FOUND"); } catch { return errorResponse(c, "INTERNAL_SERVER_ERROR"); } }); route.post("/:invitationId/accept", async c => { try { const db=deps.createDb(c.env.DATABASE_URL); const spaceId=await deps.acceptLinkedInvitation(db,c.get("currentUserId"),c.req.param("invitationId")!); if(spaceId)return c.json({data:{spaceId}}); return await deps.invitationCountryMatches(db,c.get("currentUserId"),c.req.param("invitationId")!) ? errorResponse(c,"INVITATION_NOT_FOUND") : errorResponse(c,"SPACE_COUNTRY_MISMATCH"); } catch { return errorResponse(c, "INTERNAL_SERVER_ERROR"); } }); return route; }
